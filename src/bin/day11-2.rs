        ...
        for row in 0..=rows {
            for col in 0..=cols {
                ...
                for direction in MOVES.iter() {
                    for multiplier in 1..=rows {
                        let neigh_col = col as i16 + direction.0 * multiplier as i16;
                        let neigh_row = row as i16 + direction.1 * multiplier as i16;

                        if neigh_col >= 0
                            && neigh_col <= cols as i16
                            && neigh_row >= 0
                            && neigh_row <= rows as i16
                        {
                            if seatmap[neigh_row as usize][neigh_col as usize] == '#' {
                                adj_occupied += 1;
                                break;
                            }
                            if seatmap[neigh_row as usize][neigh_col as usize] == 'L' {
                                break;
                            }
                        }
                    }
