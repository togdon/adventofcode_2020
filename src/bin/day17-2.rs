...
fn main() {
    ...
    for (row_index, row) in data.iter().enumerate() {
        for (column_index, cube) in row.chars().enumerate() {
            if cube == '#' {
                cube_map.insert((0 as i8, column_index as i8, row_index as i8, 0 as i8));
            }
        }
    }
    ...
    for cycle in 1..=6 {
        ...
        for (time, column, row, layer) in &cube_map {
            let w = *time  as i8;
            let x = *column as i8;
            let y = *row as i8;
            let z = *layer as i8;
            for (dw, dx, dy, dz) in iproduct!(-1..=1, -1..=1, -1..=1, -1..=1) {
                if (dw, dx, dy, dz) != (0, 0, 0, 0) {
                    let counter = neighbor_map.entry((w + dw, x + dx, y + dy, z + dz)).or_insert(0);
                    *counter += 1;
                }
            }
        }
